...
public class A형_새로운게임2 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < K; i++) {
			...
			list.add(new Horse(x, y, dir)); // 리스트에는 각 horse 의 정보가 담겨있음 / idx는 1번부터 시작함.
			map[x][y][0] = i + 1; // 말의 번호는 1부터 시작함.
		}
		...
		while (ans < 1000) {
			...
			for (int idx = 1; idx < len; idx++) {
				...
				if(ax < 1 || ax > N || ay < 1 || ay > N || Matrix[ax][ay] == 2) {
					if(dir == 0) { // 이동하고 list에 있는 값들 다시 변경해줘야함 !!!
						dir = 1;
					}else if(dir == 1) {
						dir = 0;
					}else if(dir == 2) {
				...
				if(Matrix[ax][ay] == 0) {
					int afterIdx = 0;
					int beforeIdx = 0;
					for(int i = 0 ; i < K ; i++) { // 이동할 칸 가장 높은 위치 찾기 
						if(map[ax][ay][i] == 0) {
							afterIdx = i;
							break;
						}
