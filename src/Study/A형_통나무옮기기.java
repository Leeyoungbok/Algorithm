...
public class A형_통나무옮기기 {
	...
	public static void main(String[] args) throws IOException{
		...
		map = new int[N+1][N+1];
		used = new boolean[N+1][N+1][2]; // 0인경우  : 가로 체크  , 1인경우 : 세로 체크
		int BCnt = 0;
		int BCur = 0;
		for(int i = 1 ; i <= N ; i++) {
	...
	static boolean bfs() {
		while(!queue.isEmpty()) {
			...
			for(int s = 0 ; s < size ; s++) {
				...
				for(int k = 0 ; k < 4 ; k++) { // turn하지 않고 그냥 이동하는 경우
					int ax = tree.x + dx[k];
					int ay = tree.y + dy[k];
				...
				if(isPossibleTurn(tree.x, tree.y, tree.cur)) { // turn 하고 진행할 수 있는지 검사하고 turn 한다음 진행
					int changeCur = 0;
					if(tree.cur == 0)
						changeCur = 1;
